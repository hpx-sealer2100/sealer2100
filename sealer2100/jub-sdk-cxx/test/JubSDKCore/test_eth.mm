...
void           transaction_Uniswap_ETH(NSUInteger contextID, Json::Value root);
void          transaction_Contract_ETH(NSUInteger contextID, Json::Value root, int index);
void      transaction_PersonalSign_ETH(NSUInteger contextID, Json::Value root);
void     transaction_TypedDataSign_ETH(NSUInteger contextID, Json::Value root);
...
void ETH_test(NSUInteger deviceID, const char* json_file) {
    ...
    while (true) {
        ...
        NSLog(@"| 5. transaction_Uniswap_test.           |\n");
        NSLog(@"|                                        |\n");
        NSLog(@"|191. personal_sign_test.                |\n");
        NSLog(@"|712. typed_data_sign_test.              |\n");
        NSLog(@"|                                        |\n");
        NSLog(@"| 7. set_my_address_test.                |\n");
        NSLog(@"| 8. set_timeout_test.                   |\n");
        NSLog(@"|                                        |\n");
        NSLog(@"| 9. return.                             |\n");
        NSLog(@"|----------------------------------------|\n");
        ...
        switch (choice) {
            ...
            case 4:
                ...
                break;
            case 5:
                transaction_Uniswap_ETH(contextID, root);
                break;
            case 191:
                transaction_PersonalSign_ETH(contextID, root);
                break;
            case 712:
                transaction_TypedDataSign_ETH(contextID, root);
                break;
            case 7:
...
void transaction_ERC20_ETH(NSUInteger contextID, Json::Value root) {
    ...
    [g_sdk JUB_SetERC20TokensETH:contextID
                      tokenArray:tokens];
    lastError = (long)[g_sdk JUB_LastError];
    NSLog(@"[%li] JUB_SetERC20TokensETH(%li)\n", lastError, contextID);
    if (JUBR_OK != lastError) {
        return ;
    }
    ...
    NSString* abi = [g_sdk JUB_BuildERC20TransferAbiETH:contextID
                                                tokenTo:tokenTo
                                             tokenValue:tokenValue];
    lastError = (long)[g_sdk JUB_LastError];
    NSLog(@"[%li] JUB_BuildERC20TransferAbiETH(%li)\n", lastError, contextID);
    if (JUBR_OK != lastError) {
        return ;
    }
    NSLog(@"abi: %@\n", abi);
...
ERC20TokenInfo* JSON2ERC20TokenInfo(int i, Json::Value root) {
    ERC20TokenInfo* token = [[ERC20TokenInfo alloc] init];
    
    NSString* uniswip = [NSString stringWithUTF8String:(char*)root["Uniswap"]["method"].asCString()];
    
    token.tokenName = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["tokens"][i]["tokenName"].asCString()];
    token.unitDP = root["Uniswap"][[uniswip UTF8String]]["tokens"][i]["dp"].asInt();
    token.contractAddress = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["tokens"][i]["contract_address"].asCString()];
    
    return token;
}


//transaction_Uniswap_ETH
void transaction_Uniswap_ETH(NSUInteger contextID, Json::Value root) {
    NSUInteger lastError = JUBR_ERROR;
    ...
    NSUInteger count = root["Uniswap"][[uniswip UTF8String]]["tokens"].size();
    if (0 >= count) {
        return;
    }
    ...
    for (int i = 0; i < count; ++i) {
        ERC20TokenInfo* token = JSON2ERC20TokenInfo(i, root);
        [ts addObject:token];
    }
    NSArray* tokens = [NSArray arrayWithArray:ts];
    
    if (1 == count) {
        [g_sdk JUB_SetERC20TokenETH:contextID
                          tokenName:[tokens[0] tokenName]
                             unitDP:[tokens[0] unitDP]
                    contractAddress:[tokens[0] contractAddress]];
        lastError = (long)[g_sdk JUB_LastError];
        NSLog(@"[%li] JUB_SetERC20TokenETH(%li)\n", lastError, contextID);
    }
    else {
        [g_sdk JUB_SetERC20TokensETH:contextID
                          tokenArray:tokens];
    ...
    //ETH Uniswap Test
    BIP32Path* path = [[BIP32Path alloc] init];
    if (root["Uniswap"]["bip32_path"]["change"].asBool()) {
        path.change = BOOL_NS_TRUE;
    }
    else {
        path.change = BOOL_NS_FALSE;
    }
    path.addressIndex = root["Uniswap"][[uniswip UTF8String]]["bip32_path"]["addressIndex"].asUInt();
    
    NSUInteger nonce = root["Uniswap"][[uniswip UTF8String]]["nonce"].asUInt();//.asDouble();
    NSUInteger gasLimit = root["Uniswap"][[uniswip UTF8String]]["gasLimit"].asUInt();//.asDouble();
    NSString* gasPriceInWei = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["gasPriceInWei"].asCString()];
    NSString* valueInWei = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["valueInWei"].asCString()];
    NSString* to = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["to"].asCString()];
    NSString* data = [NSString stringWithUTF8String:(char*)root["Uniswap"][[uniswip UTF8String]]["data"].asCString()];
...
void transaction_PersonalSign_ETH(NSUInteger contextID, Json::Value root) {
    ...
    NSString* data = [NSString stringWithUTF8String:(char*)root["Bytestring"]["data"].asCString()];
    ...
    NSString* signature = [g_sdk JUB_SignBytestringETH:contextID
                                                  path:path
                                                  data:data];
    lastError = (long)[g_sdk JUB_LastError];
    NSLog(@"[%li] JUB_SignBytestringETH(%li)\n", lastError, contextID);
    if (JUBR_OK != lastError) {
        return ;
    }
    NSLog(@"signature: %@\n", signature);
...
void transaction_TypedDataSign_ETH(NSUInteger contextID, Json::Value root) {
    ...
    NSString* fileName = [NSString stringWithUTF8String:(char*)root["EIP712"]["file"].asCString()];
    NSString* extName = [NSString stringWithUTF8String:(char*)root["EIP712"]["ext"].asCString()];
    ...
    NSString* dataInJSON = [NSString stringWithContentsOfFile:filePath
    ...
    JUB_NS_ENUM_BOOL bMetamaskV4Compat = (JUB_NS_ENUM_BOOL)root["EIP712"]["metamask_v4_compat"].asBool();
    
    NSString* signature = [g_sdk JUB_SignTypedDataETH:contextID
                                                 path:path
                                                 data:dataInJSON
                                   isMetamaskV4Compat:bMetamaskV4Compat];
    lastError = (long)[g_sdk JUB_LastError];
    NSLog(@"[%li] JUB_SignTypedDataETH(%li)\n", lastError, contextID);
    if (JUBR_OK != lastError) {
        return ;
    }
    NSLog(@"signature: %@\n", signature);
...
//    }
    ...
    case 52:
    {
        ...
        if (1 == i) {
            ...
            data = [g_sdk JUB_BuildERC20TransferAbiETH:contextID
                                               tokenTo:tokenTo
                                            tokenValue:tokenValue];
            lastError = (long)[g_sdk JUB_LastError];
            NSLog(@"[%li] JUB_BuildERC20TransferAbiETH(%li)\n", lastError, contextID);
