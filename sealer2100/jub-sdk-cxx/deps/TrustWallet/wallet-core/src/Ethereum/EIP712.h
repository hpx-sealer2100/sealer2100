...
//    static std::vector<uint8_t> struct_type_hash(const std::string& st_name, const nlohmann::json& st);
    ...
    //max function
    static bool is_array(const std::string& type_name);
    static std::optional<uint32_t> parse_array_n(const std::string& type_name);
    static std::string typeof_array(const std::string& type_name);
    static uint32_t get_byte_size_for_int_type(const std::string& type_name);
    static std::optional<uint32_t> parse_bytes_n(const std::string& type_name);
    static std::vector<uint8_t> encode_data(const std::string& type_name, const nlohmann::json& json_value);
}; // class EIP712 end
