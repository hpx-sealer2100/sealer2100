...
CompiledInstruction CompiledInstruction::Compile(const Instruction &instruction,
    ...
    auto accountIndex = [&](const Address &address) -> uint8_t {
...
void Message::compileAccounts() {
    ...
    header.numRequiredSignatures = signedAccounts.size();
    header.numCreditOnlySignedAccounts = 0;
    header.numCreditOnlyUnsignedAccounts = readOnlyAccounts.size();
...
//    };
    ...
    header.numRequiredSignatures = *p++;
    header.numCreditOnlySignedAccounts = *p++;
    header.numCreditOnlyUnsignedAccounts = *p++;
    ...
    std::transform(pubKeys.begin(), pubKeys.end(), std::back_inserter(accounts),
                   [](PublicKey &key) { return Address{key}; });
