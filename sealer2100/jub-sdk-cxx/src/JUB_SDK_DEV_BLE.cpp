...
JUB_RV JUB_initDevice(IN DEVICE_INIT_PARAM param) {
    ...
    auto bleDevice1 = Singleton<jub::JubiterBLENusDevice>::GetInstance();
    if (!bleDevice1) {
        return JUBR_ERROR;
    }

    JUB_VERIFY_RV(bleDevice1->Initialize({param.param,
                                         param.callBack,
                                         param.scanCallBack,
                                         param.discCallBack})
    );
...
JUB_RV JUB_stopEnumDevices(void) {
    ...
    JUB_VERIFY_RV(bleDevice1->StopScan());
...
JUB_RV JUB_connectDevice(JUB_BYTE_PTR devName,
...
#ifdef BLE_MODE
    //这个函数应该Android与IOS一样的代码

    jub::JubiterBLEDevice* bleDevice = nullptr;
    if (jub::JubiterSealer2100Impl::ConfirmType(devName, bBLEUUID)){
        bleDevice = Singleton<jub::JubiterBLENusDevice>::GetInstance();
        gtype = 1;
    } else {
        bleDevice = Singleton<jub::JubiterBLEDevice>::GetInstance();
        gtype = 2;
    }
    if (!bleDevice) {
        return JUBR_ERROR;
    ...
    *pDeviceID = jub::BLE_device_map::GetInstance()->AddOne(pdevHandle);
    ...
    else if (jub::JubiterSealer2100Impl::ConfirmType(devName, bBLEUUID)) {
        jub::JubiterSealer2100Impl* token = new jub::JubiterSealer2100Impl(bleDevice);
...
JUB_RV JUB_cancelConnect(JUB_BYTE_PTR devName, JUB_BYTE_PTR bBLEUUID) {
    ...
    if(gtype == 1)
    {
        ...
        JUB_VERIFY_RV(bleDevice1->CancelConnect(devName, bBLEUUID));
...
JUB_RV JUB_disconnectDevice(JUB_UINT16 deviceID) {
    ...
    JUB_ULONG *devHandle = jub::BLE_device_map::GetInstance()->GetOne(deviceID);
