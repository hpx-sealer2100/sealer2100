...
JUB_RV buildScriptPubFromAddress(const std::string& address, uchar_vector& scriptPub) {
    ...
    if (pubkeyDataBit5.first == "bitcoincash") {
        ...
        scriptLen = hyp_base58_decode_check(address.c_str(), HasherType::HASHER_SHA2D, script, scriptLen);
        if (scriptLen > 0) {
    ...
    if (rv) {
        ...
        if (   dec.first != std::string(segwit_hrl)
            && dec.first != std::string(segwit_hrl_testnet)
...
JUB_RV deserializeTx(bool bWitness,
    ...
    *plockTime = 0;
    ...
    for (auto i=0; i<outputCount; ++i) {
        ...
        strsize = hyp_base58_encode_check(script, scriptLen, HasherType::HASHER_SHA2D, str, strsize);
        delete[] script; script = NULL;
        if (strsize <= 0) {
            delete[] str; str = NULL;
            return JUBR_ARGUMENTS_BAD;
