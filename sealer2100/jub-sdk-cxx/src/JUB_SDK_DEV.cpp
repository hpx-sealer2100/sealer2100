...
JUB_RV JUB_GetDeviceType(IN JUB_UINT16 deviceID,
    ...
    else if (typeid(jub::JubiterSealer2100Impl) == typeid(*token)) {

        *commode = JUB_ENUM_COMMODE::BLE;

        *deviceClass = JUB_ENUM_DEVICE::Sealer2100;
    }
    else {
...
/*****************************************************************************
 * @function name : JUB_GetDeviceSignData
 * @in  param : deviceID - device ID
 * @out param :
 * @last change :
 *****************************************************************************/
JUB_RV JUB_GetDeviceSignData(IN JUB_UINT16 deviceID,
    ...
    std::string str_signData;
    JUB_VERIFY_RV(token->GetDeviceSignData(hashData,str_signData));
    JUB_VERIFY_RV(_allocMem(SignData, str_signData));
...
/*****************************************************************************
 * @function name : JUB_SendOnePbMessage
 * @in  param : deviceID - device ID
 *            : pbMessage - one protobuf message
 * @out param : response
 * @last change :
 *****************************************************************************/
JUB_COINCORE_DLL_EXPORT
JUB_RV JUB_SendOnePbMessage(IN JUB_UINT16 deviceID,
                            IN JUB_CHAR_PTR pbMessage ,
                            OUT JUB_CHAR_PTR_PTR response) {
    ...
    std::string str_response;
    JUB_VERIFY_RV(token->SendOnePbMessage(pbMessage, str_response));
    JUB_VERIFY_RV(_allocMem(response, str_response));

    return JUBR_OK;
}
/*****************************************************************************
 * @function name : JUB_SetLable
 * @in  param : deviceID - device ID
 *            : Lable - device lable
 * @out param :
 * @last change :
...
JUB_RV JUB_SetLable(IN JUB_UINT16 deviceID, IN JUB_CHAR_PTR pLable)
{
    ...
    auto token = jub::TokenManager::GetInstance()->GetOne(deviceID);
    JUB_CHECK_NULL(token);
    JUB_VERIFY_RV(token->IrisSetLable(pLable));
    return JUBR_OK;
}
/*****************************************************************************
 * @function name : JUB_UsePassphrase
 * @in  param : deviceID - device ID
 *            : bUse - use passphrase
 * @out param :
 * @last change :
...
JUB_RV JUB_UsePassphrase(IN JUB_UINT16 deviceID, IN bool bUse)
{
    ...
    auto token = jub::TokenManager::GetInstance()->GetOne(deviceID);
    JUB_CHECK_NULL(token);
    JUB_VERIFY_RV(token->IrisUsePassphrase(bUse));
    return JUBR_OK;
}

/*****************************************************************************
 * @function name : JUB_GetFeatures
 * @in  param : deviceID - device ID
 * @out param : Features - device Features,json format
 * @last change :
 *****************************************************************************/
JUB_COINCORE_DLL_EXPORT
                       ...
                       OUT JUB_CHAR_PTR_PTR Features)
{
    ...
    JUB_VERIFY_RV(token->IrisGetFeatures(str_Features));
    JUB_VERIFY_RV(_allocMem(Features, str_Features));
    return JUBR_OK;
}

/*****************************************************************************
 * @function name : JUB_EndSession
 * @in  param : deviceID - device ID *
 * @out param :
 * @last change :
 *****************************************************************************/
...
JUB_RV JUB_EndSession(IN JUB_UINT16 deviceID)
{
    ...
    auto token = jub::TokenManager::GetInstance()->GetOne(deviceID);
    JUB_CHECK_NULL(token);
    JUB_VERIFY_RV(token->IrisEndSession());
    return JUBR_OK;
}
/*****************************************************************************
 * @function name : JUB_OpCancel
 * @in  param : deviceID - device ID *
 * @out param :
 * @last change :
 *****************************************************************************/
...
JUB_RV JUB_OpCancel(IN JUB_UINT16 deviceID)
{
    ...
    auto token = jub::TokenManager::GetInstance()->GetOne(deviceID);
    JUB_CHECK_NULL(token);
    JUB_VERIFY_RV(token->IrisOpCancel());
    return JUBR_OK;
}
