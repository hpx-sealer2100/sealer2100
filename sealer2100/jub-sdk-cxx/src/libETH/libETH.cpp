...
JUB_RV serializeTx(const std::vector<JUB_BYTE>& vNonce,
    ...
    // 去除前导零的辅助函数
    auto removeLeadingZeros = [](const std::vector<JUB_BYTE>& data) -> std::vector<JUB_BYTE> {
        size_t start = 0;
        // 找到第一个非零字节，但要保留至少一个字节（如果是全零的情况）
        ...
        return std::vector<JUB_BYTE>(data.begin() + start, data.end());
    };
