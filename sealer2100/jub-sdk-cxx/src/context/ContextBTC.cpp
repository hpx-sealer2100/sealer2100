...
JUB_RV ContextBTC::GetHDNode(const BIP32_Path& path, std::string& xpub) {
    ...
    if(dynamic_cast<JubiterSealer2100Impl*>(token)  != nullptr)
    {
        //Sealer2100，传入_coinType
        JUB_VERIFY_RV(dynamic_cast<JubiterSealer2100Impl*>(token)->GetHDNodeBTC_2100(_coinType,_transType, strPath, xpub));
    }else{
...
JUB_RV ContextBTC::GetMainHDNode(std::string& xpub) {
    ...
    {
        //Sealer2100，传入_coinType
        JUB_VERIFY_RV(dynamic_cast<JubiterSealer2100Impl*>(token)->GetHDNodeBTC_2100(_coinType,_transType, _mainPath, xpub));
    }else{
...
JUB_RV ContextBTC::GetAddress(const JUB_ENUM_BTC_ADDRESS_FORMAT& addrFmt, const BIP32_Path& path, const JUB_UINT16 tag, std::string& address) {
    ...
    if(dynamic_cast<JubiterSealer2100Impl*>(token)  != nullptr)
    {
        JUB_VERIFY_RV(dynamic_cast<JubiterSealer2100Impl*>(token)->GetAddressBTC_2100(_coinType,addrFmt, _transType, strPath, tag, address));
    }else{
...
//    vContractAddress.erase(vContractAddress.begin());
    ...
    toAddressLen = hyp_base58_decode_check(to, HasherType::HASHER_SHA2D, toAddress, toAddressLen);
    if (toAddressLen > 0) {
...
JUB_RV ContextBTC::SignTX(const JUB_ENUM_BTC_ADDRESS_FORMAT& addrFmt, const std::vector<INPUT_BTC>& vInputs, const std::vector<OUTPUT_BTC>& vOutputs, const JUB_UINT32 lockTime, std::string& raw) {
    ...
    if(dynamic_cast<JubiterSealer2100Impl*>(token)  != nullptr)
    {
        //Sealer2100，不需要再组织未签名交易，直接传入
        uchar_vector vRaw;
        JUB_VERIFY_RV(dynamic_cast<JubiterSealer2100Impl*>(token)->SignTXBTC_2100(_coinType,_mainPath,addrFmt,vInputs,vOutputs,lockTime,vRaw));
        raw = vRaw.getHex();
        return JUBR_OK;
    }
...
JUB_RV ContextMultisigBTC::GetAddress(const JUB_ENUM_BTC_ADDRESS_FORMAT& addrFmt, const BIP32_Path& path, const JUB_UINT16 tag, std::string& address) {
    ...
    if(dynamic_cast<JubiterSealer2100Impl*>(token)  != nullptr)
    {
        //Sealer2100，用trezor方式传输参数
       //JUB_VERIFY_RV(dynamic_cast<JubiterSealer2100Impl*>(token)->GetAddressMultisigBTC_2100(addrFmt, _transType, strPath, _vCosignerMainxPub,_m,_n, tag, address));
       // return JUBR_OK;
    }
...
JUB_RV ContextMultisigBTC::GetP2SHMultisigAddress(const BIP32_Path& path, const JUB_UINT16 tag, std::string& address) {
    ...
    strsize = hyp_base58_encode_check(scp, scpLen, HasherType::HASHER_SHA2D, str, strsize);
    delete[] scp; scp = NULL;
    if (strsize > 0) {
...
JUB_RV ContextMultisigBTC::SignTX(const JUB_ENUM_BTC_ADDRESS_FORMAT& addrFmt, const std::vector<INPUT_BTC>& vInputs, const std::vector<OUTPUT_BTC>& vOutputs, const JUB_UINT32 lockTime, std::string& raw) {
    ...
    {
        //Sealer2100，不需要再组织未签名交易，直接传入
        //return dynamic_cast<JubiterSealer2100Impl*>(token)->SignTXBTC_2100(_coinType,_mainPath,addrFmt,vInputs,vOutputs,lockTime,raw);
        return JUBR_OK;
    }
