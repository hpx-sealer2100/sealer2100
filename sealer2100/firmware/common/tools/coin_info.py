...
def _load_supported_ethereum_networks() -> Coins:
    evm_supported = DEFS_DIR / "evm-supported.csv"
    with open(evm_supported) as f:
        csv_reader = csv.DictReader(f, delimiter=',', skipinitialspace=True)
        coins = [{**item, 'chain_id': int(item['chain_id'])} for item in csv_reader]
        return coins

def _load_ethereum_networks() -> Coins:
    """Load ethereum networks from `ethereum/networks.json`"""
    supported = _load_supported_ethereum_networks()
    supported_chainid = [item['chain_id'] for item in supported]
    ...
    for chain in sorted(
        ...
        if not chain_data["chainId"] in supported_chainid:
            continue
        ...
        if is_testnet:
            ...
            # 客户要求全部为60
            slip44 = 60 #chain_data.get("slip44", 60)
        ...
        network = dict(
            ...
            slip44=slip44,
            shortcut=shortcut,
            name= EVM_NAME_OVERRIDE.get(chain_data["chainId"], name),
            rskip60=rskip60,
...
def _load_erc20_tokens() -> Coins:
    ...
    def network_for_chain_id(chain_id: int) -> str:
        for network in networks:
            if network.get('chain_id') == chain_id:
                return network.get('chain')
    ...
    for file in sorted(chain_path.glob("*.csv")):
        with open(file) as f:
            reader = csv.DictReader(f, skipinitialspace=True)
            # chain_id	symbol	name	adress	decimals
            for token in reader:
                chain_id = int(token['chain_id'])
                chain = network_for_chain_id(chain_id)

                symbol = token['symbol']
                name = token['name'] or symbol
                address = token['address']
                decimals = int(token['decimals'])
                key = f"erc20:{chain.lower()}:{symbol}"
                tokens.append(Coin(
                    name=name,
                    chain=chain,
                    chain_id=chain_id,
...
def _load_solana_tokens():
    """Load solana tokens from """
    tokens: Coins = []
    with open(DEFS_DIR/"solana"/"tokens.csv") as f:
...
def _load_tron_tokens():
    ...
    tokens: Coins = []
    with open(DEFS_DIR/"tron"/"tokens.csv") as f:
        reader = csv.DictReader(f)
        for token in reader:
