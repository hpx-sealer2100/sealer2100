...
{
    bool is_inited;
    bool is_mounted_sys_data;
    FATFS fs_obj_sys_data;
...
// macros
#define EMMC_DBG_STATUS_SETUP()                                 \
  {                                                               \
    strncpy(emmc_wrapper_status.from_function, __FUNCTION__, 64); \
    emmc_wrapper_status.ff_call[0] = '\0';                        \
  }

#define ExecuteCheck_EMMC_ADVANCED(func_call, expected_condition, on_false) \
  {                                                                           \
...
#define ExecuteCheck_EMMC_SIMPLE(func_call)                                      \
  {                                                                                \
    ExecuteCheck_EMMC_ADVANCED(func_call, (EC_res == FR_OK), { return false; }); \
  }
