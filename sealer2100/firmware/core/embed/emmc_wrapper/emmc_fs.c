...
void emmc_fs_format_status(char* str, size_t str_len)
{
    ...
    snprintf(
        str, str_len,
        "\nis_inited=%s\nis_mounted_sys_data=%s\nis_mounted_user_data=%s\nfrom_function=%s\nff_call=%s\nff_result=%x\ndebug_extra=%s\n",
        emmc_wrapper_status.is_inited?"True":"False",
        emmc_wrapper_status.is_mounted_sys_data?"True":"False",
...
bool emmc_fs_recreate(bool partition_table, bool sys_data, bool user_data)
{
    // status update
    EMMC_DBG_STATUS_SETUP();

    // reset status
    emmc_wrapper_status.is_mounted_sys_data = false;
    ...
    if ( partition_table )
    {
        ExecuteCheck_EMMC_SIMPLE(f_fdisk(0, plist, work));
    }
    ...
    if ( partition_table || sys_data )
    {
        ExecuteCheck_EMMC_SIMPLE(f_mkfs("0:", &mk_para, work, sizeof(work)));
        ExecuteCheck_EMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_sys_data, "0:", 1));

        // set label
        ExecuteCheck_EMMC_SIMPLE(f_setlabel("0:System Data"));

        // create folders and files to avoid stuck in boardloader
        // this should be kept if we switch to use command to upload files
        FIL res_flag;
        ExecuteCheck_EMMC_SIMPLE(f_mkdir("0:res"));
        ExecuteCheck_EMMC_SIMPLE(f_close(&res_flag));
        ExecuteCheck_EMMC_SIMPLE(f_mkdir("0:boot"));

        ExecuteCheck_EMMC_SIMPLE(f_unmount("0:"));
    }
    if ( partition_table || user_data )
    {
        ExecuteCheck_EMMC_SIMPLE(f_mkfs("1:", &mk_para, work, sizeof(work)));
        ExecuteCheck_EMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_userData, "1:", 1));

        ExecuteCheck_EMMC_SIMPLE(f_setlabel("1:User Data"));

        ExecuteCheck_EMMC_SIMPLE(f_unmount("1:"));
    }
...
bool emmc_fs_is_partitioned()
{
    ...
    FATFS fs_obj_test;
    ExecuteCheck_EMMC_SIMPLE(f_mount(&fs_obj_test, "0:", 1));
    ExecuteCheck_EMMC_SIMPLE(f_unmount("0:"));
    return true;
}
bool emmc_fs_mount(bool sys_data, bool user_data)
{
    // status update
    EMMC_DBG_STATUS_SETUP();
    ...
    {
        ExecuteCheck_EMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_sys_data, "0:", 1));
        emmc_wrapper_status.is_mounted_sys_data = true;
    }
    if ( user_data )
    {
        ExecuteCheck_EMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_userData, "1:", 1));
...
bool emmc_fs_mount_check(void)
{
    ...
    if ( !emmc_wrapper_status.is_inited )
        return false;
    res = f_mount(&emmc_wrapper_status.fs_obj_sys_data, "0:", 1);
...
bool emmc_fs_unmount(bool sys_data, bool user_data)
{
    // status update
    EMMC_DBG_STATUS_SETUP();
...
bool emmc_fs_fix_permission_internal(char* path_buff)
{
    ...
    ExecuteCheck_EMMC_SIMPLE(f_opendir(&dir, path_buff));
    int i = strlen(path_buff);
    for ( ;; )
    {
        // read next file
        ExecuteCheck_EMMC_SIMPLE(f_readdir(&dir, &finfo));
        ...
        ExecuteCheck_EMMC_SIMPLE(f_chmod(path_buff, 0, AM_RDO | AM_HID | AM_SYS | AM_ARC));
    ...
    ExecuteCheck_EMMC_SIMPLE(f_closedir(&dir));

    // no error, all done
    return true;
}

bool emmc_fs_fix_permission(bool sys_data, bool user_data)
{
    // status update
    EMMC_DBG_STATUS_SETUP();
    ...
    if ( sys_data )
    {
        if ( !emmc_wrapper_status.is_mounted_sys_data )
            return false;
        strcpy(path_buff, "0:");
...
)
{
    ...
    // open
    ExecuteCheck_EMMC_SIMPLE(f_open(&fhandle, path_buff, FA_READ));
    // seek
    ExecuteCheck_EMMC_SIMPLE(f_lseek(&fhandle, offset));
    // read
    ExecuteCheck_EMMC_SIMPLE(f_read(&fhandle, buff, target_len, (UINT*)processed_len));
    // close
    ExecuteCheck_EMMC_SIMPLE(f_close(&fhandle));
...
)
{
    ...
    case FR_OK: // exists
        ...
        {
            // overwrite enabled
            ExecuteCheck_EMMC_SIMPLE(f_unlink(path_buff)); // remove it
    ...
    // open
    ExecuteCheck_EMMC_SIMPLE(f_open(&fhandle, path_buff, fopenmodes));
    // seek
    ExecuteCheck_EMMC_SIMPLE(f_lseek(&fhandle, offset));
    // read
    ExecuteCheck_EMMC_SIMPLE(f_write(&fhandle, buff, target_len, (UINT*)processed_len));
    // flush (not needed as f_close should flush the buffer)
    // ExecuteCheck_EMMC_SIMPLE(f_sync(&fhandle));
    // close
    ExecuteCheck_EMMC_SIMPLE(f_close(&fhandle));
...
bool emmc_fs_dir_create_internal(char* path_buff)
{
    ...
    case FR_NO_FILE: // if path not exists, create it
        ExecuteCheck_EMMC_SIMPLE(f_mkdir(path_buff));
        break;
    default: // anything else considered as error
        return false;
        break;
...
bool emmc_fs_dir_make(char* path_buff)
{
    ...
    FILINFO finfo;
    ExecuteCheck_EMMC_SIMPLE(emmc_fs_f_stat(path_buff, &finfo));
    if ( finfo.fattrib & AM_DIR )
        return true;
    else
        return false;
...
bool emmc_fs_dir_delete_internal(char* path_buff)
{
    ...
    for ( ;; )
    {
        ...
        ExecuteCheck_EMMC_SIMPLE(f_chmod(path_buff, 0, AM_RDO));
        ...
        {
            // delete directly
            ExecuteCheck_EMMC_SIMPLE(f_unlink(path_buff));
        }
