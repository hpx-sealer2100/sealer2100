...
// 定义一个数组并放置到 SRAM4
__attribute__((section(".emmc_data"))) uint8_t bl_data_buffer[64*1024];
bootloader_buffer* bl_buffer = (bootloader_buffer*)bl_data_buffer;
// SDRAM BUFFER
uint8_t *bl_update_buffer = (uint8_t *)FMC_SDRAM_BOOLOADER_BUFFER_ADDRESS;
// UI progesss
...
int process_msg_FirmwareUpdateEmmc(uint8_t iface_num, uint32_t msg_size, uint8_t* buf)
{
    ...
    ExecuteCheck_MSGS_ADV(
        emmc_fs_file_read(msg_recv.path, 0, bl_update_buffer, emmc_file_size, &processed), true,
        {
#if PRODUCTION
            send_failure_detailed(
                iface_num, FailureType_Failure_ProcessError, "%s -> %s Failed", __func__, str_func_call
    ...
    else if ( memcmp(bl_update_buffer, "5283", 4) == 0 )
    {
        ...
        ExecuteCheck_MSGS_ADV(
            load_ble_image_header(
                bl_update_buffer, FIRMWARE_IMAGE_MAGIC_BLE, FIRMWARE_IMAGE_MAXSIZE_BLE, &file_hdr
            ),
            sectrue,
            {
                emmc_fs_file_delete(msg_recv.path);
    ...
    else
    {
        ...
        while ( true )
        {
            ...
            if ( 0 == memcmp(hash1, hash2, 32) )
            {
                ...
                // compare version
                if ( !(version_compare(file_hdr.hypermate_version, current_hdr.hypermate_version) < 0) )
                {
                  ...
                  sector_offset += 32 )
            {
                ...
                ExecuteCheck_MSGS_ADV_RETRY_DELAY(
                    flash_write_words(
                        ...
                        (uint32_t*)(bl_update_buffer + processed_bytes)
                    ),
                    sectrue,
                    {
...
{
    // fix hypermate data and user data
