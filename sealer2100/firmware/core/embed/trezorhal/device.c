...
void device_burnin_test(bool force) {
  ...
  volatile uint32_t motor_pre = 0, motor_now = 0;
  ...
  do {
    ...
    if (index != index_bak) {
      ...
      switch (index) {
        ...
        case 3:
          ...
          motor_pre = motor_now = __HAL_TIM_GET_COUNTER(&TimHandle);
    ...
    if ((current - previous) > (TIMER_1S / 10)) {
      ...
      if (index == 3) {
        motor_now = __HAL_TIM_GET_COUNTER(&TimHandle);
        if (motor_now - motor_pre < (TIMER_1S * 15) / 10) {
