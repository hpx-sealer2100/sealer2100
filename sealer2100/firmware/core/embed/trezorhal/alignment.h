...
/*
 * Define EFFICIENT_UNALIGNED_ACCESS for architectures where unaligned memory
 ...
 */
    ...
    || defined(ARCH_IS_X86) || defined(ARCH_IS_X64) \
    || defined(PLATFORM_IS_WINDOWS_ON_ARM64)
/*
 ...
 * device memory).
 */
#define EFFICIENT_UNALIGNED_ACCESS
#endif

#if defined(__IAR_SYSTEMS_ICC__) && \
    (defined(ARCH_IS_ARM64) || defined(ARCH_IS_ARM32) \
    || defined(__ICCRX__) || defined(__ICCRL78__) || defined(__ICCRISCV__))
#pragma language=save
#pragma language=extended
#define POP_IAR_LANGUAGE_PRAGMA
...
typedef uint16_t __packed uint16_unaligned_t;
typedef uint32_t __packed uint32_unaligned_t;
typedef uint64_t __packed uint64_unaligned_t;
#elif defined(COMPILER_IS_GCC) && (GCC_VERSION >= 40504) && \
    ((GCC_VERSION < 60300) || (!defined(EFFICIENT_UNALIGNED_ACCESS)))
/*
...
typedef struct {
    uint16_t x;
} __attribute__((packed)) uint16_unaligned_t;
typedef struct {
    uint32_t x;
} __attribute__((packed)) uint32_unaligned_t;
typedef struct {
    uint64_t x;
} __attribute__((packed)) uint64_unaligned_t;
 #endif

/*
 * We try to force (get|put)_unaligned_uintXX to be always inline, because this results
...
static inline uint16_t get_unaligned_uint16(const void *p)
{
    ...
    uint16_unaligned_t *p16 = (uint16_unaligned_t *) p;
    r = *p16;
...
__attribute__((always_inline))
#endif
static inline void put_unaligned_uint16(void *p, uint16_t x)
{
#if defined(UINT_UNALIGNED)
    uint16_unaligned_t *p16 = (uint16_unaligned_t *) p;
    *p16 = x;
...
static inline uint32_t get_unaligned_uint32(const void *p)
{
    uint32_t r;
#if defined(UINT_UNALIGNED)
    uint32_unaligned_t *p32 = (uint32_unaligned_t *) p;
    r = *p32;
#elif defined(UINT_UNALIGNED_STRUCT)
    uint32_unaligned_t *p32 = (uint32_unaligned_t *) p;
    r = p32->x;
...
static inline uint64_t get_unaligned_uint64(const void *p)
{
    uint64_t r;
#if defined(UINT_UNALIGNED)
    uint64_unaligned_t *p64 = (uint64_unaligned_t *) p;
    r = *p64;
#elif defined(UINT_UNALIGNED_STRUCT)
    uint64_unaligned_t *p64 = (uint64_unaligned_t *) p;
    r = p64->x;
...
__attribute__((always_inline))
#endif
static inline void put_unaligned_uint64(void *p, uint64_t x)
{
#if defined(UINT_UNALIGNED)
    uint64_unaligned_t *p64 = (uint64_unaligned_t *) p;
    *p64 = x;
...
#if defined(POP_IAR_LANGUAGE_PRAGMA)
...
#if __has_builtin(__builtin_bswap16) && !defined(BSWAP16)
#define BSWAP16 __builtin_bswap16
#endif /* __has_builtin(__builtin_bswap16) */
#if __has_builtin(__builtin_bswap32) && !defined(BSWAP32)
#define BSWAP32 __builtin_bswap32
#endif /* __has_builtin(__builtin_bswap32) */
#if __has_builtin(__builtin_bswap64) && !defined(BSWAP64)
#define BSWAP64 __builtin_bswap64
...
#if defined(__ARMCC_VERSION) && (__ARMCC_VERSION >= 410000) && !defined(BSWAP32)
...
#include <arm_acle.h>
#define BSWAP16(x) ((uint16_t) __rev16((uint32_t) (x)))
#define BSWAP32 __rev
#define BSWAP64 __revll
#endif
#endif
...
#else
static const uint16_t byte_order_detector = { 0x100 };
#define IS_BIG_ENDIAN (*((unsigned char *) (&byte_order_detector)) == 0x01)
#endif
...
#define GET_UINT32_BE(data, offset)                                \
    ((IS_BIG_ENDIAN)                                               \
        ? get_unaligned_uint32((data) + (offset))                  \
        : BSWAP32(get_unaligned_uint32((data) + (offset))) \
    )
...
#define PUT_UINT32_BE(n, data, offset)                                   \
    {                                                                            \
        if (IS_BIG_ENDIAN)                                               \
        {                                                                        \
            put_unaligned_uint32((data) + (offset), (uint32_t) (n));     \
        }                                                                        \
        else                                                                     \
        {                                                                        \
            put_unaligned_uint32((data) + (offset), BSWAP32((uint32_t) (n))); \
        }                                                                        \
    }
...
#define GET_UINT16_LE(data, offset)                                \
    ((IS_BIG_ENDIAN)                                               \
        ? BSWAP16(get_unaligned_uint16((data) + (offset))) \
        : get_unaligned_uint16((data) + (offset))                  \
    )
...
#define PUT_UINT16_LE(n, data, offset)                                   \
    {                                                                            \
        if (IS_BIG_ENDIAN)                                               \
        {                                                                        \
            put_unaligned_uint16((data) + (offset), BSWAP16((uint16_t) (n))); \
        }                                                                        \
        else                                                                     \
        {                                                                        \
            put_unaligned_uint16((data) + (offset), (uint16_t) (n));     \
        }                                                                        \
    }
 ...
 *                  integer from.
 */
#define GET_UINT24_BE(data, offset)        \
    (                                              \
 ...
 */
#define PUT_UINT24_BE(n, data, offset)                \
    {                                                         \
        (data)[(offset)] = BYTE_2(n);                 \
        (data)[(offset) + 1] = BYTE_1(n);             \
        (data)[(offset) + 2] = BYTE_0(n);             \
    }
 ...
 *                  integer from.
 */
#define GET_UINT24_LE(data, offset)               \
    (                                                     \
 ...
 *                  byte of the 24 bits unsigned integer \p n.
 */
#define PUT_UINT24_LE(n, data, offset)                \
    {                                                         \
...
#define GET_UINT64_BE(data, offset)                                \
    ((IS_BIG_ENDIAN)                                               \
        ? get_unaligned_uint64((data) + (offset))                  \
        : BSWAP64(get_unaligned_uint64((data) + (offset))) \
    )
...
#define PUT_UINT64_BE(n, data, offset)                                   \
    {                                                                            \
        if (IS_BIG_ENDIAN)                                               \
        {                                                                        \
            put_unaligned_uint64((data) + (offset), (uint64_t) (n));     \
        }                                                                        \
        else                                                                     \
        {                                                                        \
            put_unaligned_uint64((data) + (offset), BSWAP64((uint64_t) (n))); \
        }                                                                        \
    }
