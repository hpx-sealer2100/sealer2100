...
#define UPDATE_SE 0xA5
#define UPDATE_NOR 0xAA
#define UPDATE_EMMC 0xCC
...
bool button_request(uint8_t iface_num) {
  // send button request
  MSG_SEND_INIT(ButtonRequest);
  MSG_SEND_ASSIGN_VALUE(code, ButtonRequestType_ButtonRequest_Other);
  MSG_SEND(ButtonRequest);

  uint8_t buf[USB_PACKET_SIZE] = {0};
  uint16_t msg_id = 0;
  uint32_t msg_size = 0;
  MSG_RECV_INIT(ButtonAck);

  while (msg_size != USB_PACKET_SIZE)
  {
    if (host_channel == CHANNEL_USB) {
      // read next packet (with retry)
      msg_size = usb_webusb_read_blocking(iface_num, buf, USB_PACKET_SIZE, 5);
    } else {
      msg_size = spi_slave_poll(buf);
    }
  }
  msg_parse_header(buf, &msg_id, &msg_size);
  ...
  if (!pb_decode_noinit(&stream, ButtonAck_fields, &msg_recv)) {
    return false;
  }
  return true;
}
...
static void send_msg_features(uint8_t iface_num,
  ...
  if (device_is_factory_mode()) {
    ...
    MSG_SEND_ASSIGN_STRING(vendor, "hypermate.com");
    ...
    if (vhdr && hdr) {
      ...
      MSG_SEND_ASSIGN_STRING_LEN(hypermate_version, ver_str, 5);
    } else {
    ...
    MSG_SEND_ASSIGN_VALUE(battery, battery_read_SOC());
  }
...
static void detect_installation(vendor_header *current_vhdr,
  ...
  *is_downgrade_wipe = secfalse;
  ...
  if (version_compare(new_hdr->hypermate_version, current_hdr->hypermate_version) <
      0) {
