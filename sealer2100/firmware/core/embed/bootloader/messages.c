...
static uint8_t msg_read_state = READSTATE_IDLE;
static uint8_t msg_out[MSG_OUT_BUFFER_SIZE];
_Static_assert(MSG_OUT_BUFFER_SIZE % USB_PACKET_SIZE == 0, "MSG_OUT_BUFFER_SIZE");
extern int bip39_cache_index;

/*增加初始化静态变量*/
void msg_init_static_para(void) {
  msg_read_state = READSTATE_IDLE;
  msg_out_start = 0x00;
  msg_out_end = 0x00;
  msg_out_cur = 0x00;
  bip39_cache_index = 0x00;
}
...
bool pb_read_common(const pb_msgdesc_t *fields,
                    //                    const pb_byte_t pb[MSG_IN_ENCODED_SIZE], size_t
                    //                    pb_len,
                    const pb_byte_t *pb, size_t pb_len, void *msg_ptr) {
  ...
  return pb_decode(&stream, fields, msg_ptr);
}

size_t pb_write_common(const pb_msgdesc_t *fields, const void *msg_ptr,
                       //                       pb_byte_t pb[MSG_IN_ENCODED_SIZE]) {
                       pb_byte_t *pb) {
  size_t len = 0;
  ...
  if (!pb_encode(&stream, fields, msg_ptr)) {
    return 0;
  }
...
bool msg_write_common(char type, uint16_t msg_id, const void *msg_ptr) {
  ...
  // usb和ble通道准备发送数据
  boot_ctx.host.state = HOST_CHANNEL_STATE_PROCESSED;

  return status;
}

void msg_process(char type, uint16_t msg_id, const pb_msgdesc_t *fields, uint8_t *msg_encoded,
...
volatile bool command_need_continue = false;
void msg_read_common(char type, const uint8_t *buf, uint32_t len) {
  ...
  command_need_continue = false;  // 出错和完成为false，正常继续为true

  // 参数检查
  if (len != USB_PACKET_SIZE) return;

  if (msg_read_state == READSTATE_IDLE) {
#if 0
    // 开启背光和复位空闲定时
    lcdSetBacklightOn();
    refresh_timer(timer_backlight);
#endif
  ...
  command_need_continue = true;
  if (msg_pos >= msg_encoded_size) {
    command_need_continue = false;
    // 设置当前信道为忙
    boot_ctx.host.state = HOST_CHANNEL_STATE_READING;
    msg_pos = 0;
    msg_read_state = READSTATE_IDLE;
...
void msg_read_tiny(const uint8_t *buf, int len) {
  ...
  switch (msg_id) {
    ...
    default:
      ...
      // fsm_sendFailure(FailureType_Failure_UnexpectedMessage, _("Message
      // unexpected")); 不支持的 msg id
      msg_tiny_id = MSG_UNEXPECT_ID;
      return;
  }
