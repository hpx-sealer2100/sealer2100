...
from allowed_messages import messages as allowed_messages
...
LABELS = {
    ...
    wire_bootloader: "bootloader messages",
}
...
def get_message_short_name(message):
    return message.name.split("MessageType_", 1).pop()

def handle_message(fh, fl, skipped, message, extension):
    name = message.name
    short_name = get_message_short_name(message)
...
for message in MessageType.DESCRIPTOR.values:
    if message.GetOptions().deprecated:
        print(get_message_short_name(message), 'deprecated')
        continue
    for extension in get_wire_extensions(message):
        messages[extension].append(message)

for extension in (wire_in, wire_out, wire_bootloader):
    ...
    for message in messages[extension]:
        print(get_message_short_name(message))
        if get_message_short_name(message) not in allowed_messages:
            print('not in allowed list, continue')
            continue
        print('in allowed list, construct')
