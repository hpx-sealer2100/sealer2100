...
#define SPI_IFACE (6)
#define IRIS_IFACE (250)
#define CAMERA_STATE_IFACE (251)
#define CAMERA_IFACE (252)
  ...
  for (;;) {
    ...
    while ((item = mp_iternext(iter)) != MP_OBJ_STOP_ITERATION) {
      ...
      if (false) {
        ...
        bool usb_connected = is_usb_configuard() == sectrue ? true : false;
      ...
      // emulator not have button
        ...
        bool connected = is_usb_connected() == sectrue;
        ret->items[0] = MP_OBJ_NEW_SMALL_INT(i);
        ret->items[1] = connected ? mp_const_true :mp_const_false;
        return mp_const_true;
      } else if (iface == CAMERA_STATE_IFACE) {
        // check camera is power on
        if (camera_is_power_on()) {
          if (camera_is_captured()) {
            ret->items[0] = MP_OBJ_NEW_SMALL_INT(i);
            ret->items[1] = mp_const_true;
            return mp_const_true;
          }
        }
      } else if (iface == CAMERA_IFACE) {
        // check camera is power on
        if (camera_is_power_on()) {
          if (camera_is_grayscale_done()) {
            uint8_t qrcode[1024 + 1] = {0};
            int type = -1;
            int len = camera_scan_qrcode(qrcode, &type);
            if (len > 0) {
              if (type == CAMERA_QR_TYPE_STRING) {
                ret->items[0] = MP_OBJ_NEW_SMALL_INT(i);
                ret->items[1] = mp_obj_new_str((const char*)qrcode, len);
              } else if (type == CAMERA_QR_TYPE_BYTES) {
                ret->items[0] = MP_OBJ_NEW_SMALL_INT(i);
                ret->items[1] = mp_obj_new_bytes(qrcode, len);
              } else if (type == CAMERA_QR_TYPE_NUMBER) {
              }
              return mp_const_true;
            }
          }
        ...
        } else if (iface == IRIS_IFACE) {
          uint8_t buf[64] = {0};
          int len = iris_usart_read(buf, sizeof(buf));
          if (len > 0) {
            ret->items[0] = MP_OBJ_NEW_SMALL_INT(i);
            ret->items[1] = mp_obj_new_bytes(buf, len);
            return mp_const_true;
