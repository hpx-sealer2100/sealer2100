...
if not utils.BITCOIN_ONLY:
    ...
    async def derive_and_store_roots(ctx: wire.Context) -> None:
        ...
        # store the flag for airgap walletconnect
        ctx._have_passphrase = not passphrase == ""
        if need_seed:
            from trezor.ui.layouts import wait_doing
            async def _get_seed() -> str:
                return mnemonic.get_seed(passphrase, progress_bar=False)
            common_seed = await wait_doing("processing ...", _get_seed())
    ...
    async def get_seed(ctx: wire.Context) -> bytes:
        ...
        async def _get_seed() -> str:
            return mnemonic.get_seed(passphrase, progress_bar=False)
        return await wait_doing("processing ...", _get_seed())
