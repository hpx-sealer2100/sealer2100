...
def get_vendor():
    return "trezor.io" if storage.device.is_trezor_compatible() else "hypermate.com"
...
def get_features() -> Features:
    ...
    f = Features(
        ...
        battery= io.Battery().state_of_charge()
    )
...
async def handle_Ping(ctx: wire.Context, msg: Ping) -> Success:
    if msg.button_protection:
        ...
        await confirm_action(ctx, "Confirm", "ping", br_code=B.ProtectCall)
...
async def handle_DoPreauthorized(
    ...
    handler = workflow_handlers.find_registered_handler(
        ctx.wire.iface, req.MESSAGE_WIRE_TYPE
    )
    if handler is None:
...
async def set_homescreen() -> None:
    from trezor.ui.screen import manager
    if storage.device.is_initialized():
        if not config.is_unlocked():
            from trezor.ui.screen.lockscreen import LockScreen
            cls = LockScreen
        else:
            from trezor.ui.screen.home import HomeScreen
            from trezor.ui.screen.lockscreen import ProtectScreen
            if manager.try_switch_to(ProtectScreen):
                return
            cls = HomeScreen
    else:
        from trezor.ui.screen.initialize.language import LanguageScreen
        cls = LanguageScreen
    if manager.try_switch_to(cls):
        return
    screen = cls()
    await screen.show()
...
def get_state() -> str | None:
    ...
    if storage.device.no_backup():
        dev_state = i18n.Text.seedless
    elif storage.device.unfinished_backup():
        dev_state = i18n.Text.backup_failed
    elif storage.device.needs_backup():
        dev_state = i18n.Text.need_backup
    elif not config.has_pin():
        dev_state = i18n.Text.pin_not_set
    elif storage.device.get_experimental_features():
        dev_state = i18n.Text.experimental_mode
    else:
        dev_state = None
    return dev_state
...
def screen_off_if_possible() -> None:
    ...
    if ui.display.backlight():
        if config.is_unlocked():
            ui.display.backlight(ui.BACKLIGHT_LOW)
        workflow.idle_timer.set(3 * 1000, lock_device_if_unlocked)


def shutdown_device() -> None:
    from trezor import uart, io
    if not utils.is_usb_plugged():
        if storage.device.ble_enabled():
...
async def unlock_device(ctx: wire.GenericContext = wire.DUMMY_CONTEXT) -> None:
    ...
    if not config.is_unlocked():
        ...
        screen = Popup(i18n.Text.unlocking)
        screen.auto_close_timeout = 500
        log.debug(__name__, "waiting doing")
        await screen.show()
        await screen
...
def reload_settings_from_storage() -> None:
    ...
    auto_lock = storage.device.get_autolock_delay_ms()
    auto_shutdown = storage.device.get_autoshutdown_delay_ms()

    # set auto screen off first
    if auto_lock > 0:
        workflow.idle_timer.set(auto_lock, screen_off_if_possible)
    else:
        workflow.idle_timer.remove(screen_off_if_possible)

    # if usb connected, we can't shutdown
    if utils.is_usb_plugged():
        return

    if auto_shutdown > 0:
        workflow.idle_timer.set(auto_shutdown, shutdown_device)
    else:
        workflow.idle_timer.remove(shutdown_device)

    # not never, shutdown deley less then lock, only need shutdown
    if auto_shutdown > 0 and auto_shutdown <= auto_lock:
        workflow.idle_timer.remove(screen_off_if_possible)
