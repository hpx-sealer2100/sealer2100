...
async def verify_user(keepalive_callback: KeepaliveCallback) -> bool:
    ...
    try:
        ...
        await verify_protection(cache_time_ms=_UV_CACHE_TIME_MS)
        ret = True
...
class U2fConfirmRegister(U2fState):
    ...
    async def confirm_dialog(self) -> bool:
        if self._cred.rp_id_hash in _BOGUS_APPIDS:
            if self.cid == _last_good_auth_check_cid:
                await show_popup(
                    i18n.Text.u2f_already_registered,
                    timeout_ms=_POPUP_TIMEOUT_MS,
                )
            else:
                await show_popup(
                    i18n.Text.u2f_not_registered,
                    timeout_ms=_POPUP_TIMEOUT_MS,
                )
            return False
        else:
    ...
    def get_header(self) -> str:
        return i18n.Title.u2f_register
...
class U2fConfirmAuthenticate(U2fState):
    ...
    def get_header(self) -> str:
        return i18n.Title.u2f_authenticate
...
class Fido2ConfirmMakeCredential(Fido2State, ConfirmInfo):
    ...
    def get_header(self) -> str:
        return i18n.Title.fido2_register
...
class Fido2ConfirmExcluded(Fido2ConfirmMakeCredential):
    ...
    async def on_confirm(self) -> None:
        ...
        await send_cmd(cmd, self.iface)
        self.finished = True
        msg = i18n.Text.fido2_already_registered_x.format(self._cred.rp_id)
        await show_popup(
            msg,
            timeout_ms=_POPUP_TIMEOUT_MS,
        )
...
class Fido2ConfirmGetAssertion(Fido2State, ConfirmInfo, Pageable):
    ...
    def get_header(self) -> str:
        return i18n.Title.fido2_authenticate
...
class Fido2ConfirmNoCredentials(Fido2ConfirmGetAssertion):
    ...
    async def on_confirm(self) -> None:
        ...
        await send_cmd(cmd, self.iface)
        self.finished = True
        msg = i18n.Text.device_already_registered_x.format(self._creds[0].app_name)
        await show_popup(
            msg,
            timeout_ms=_POPUP_TIMEOUT_MS,
        )
