...
if TYPE_CHECKING:
    ...
    from typing import Protocol, Any, Dict, List, Callable, TypeVar, Type

    from trezor.utils import BufferType

    D = TypeVar('D', bound='Deserializable')
    S = TypeVar('S', bound='Serializable')

    Decoder = Callable[['Deserializer'], D]
    Encoder = Callable[['Serializer', D], None]
...
class Deserializer:
    ...
    def __init__(self, data: bytes):
        self._input = BufferReader(data)

    def remaining(self) -> int:
        return self._input.remaining_count()

    def bool(self) -> bool:
        value = int.from_bytes(self._read(1), "little")
        if value == 0:
            return False
        elif value == 1:
            return True
    ...
    def struct(self, struct: Type[D]) -> D:
...
class Serializer:
    ...
    def __init__(self, buffer: BufferType = bytearray()):
        self._output = BufferWriter(buffer)

    def output(self) -> bytes:
        return bytes(self._output.buffer)
    ...
    def _write_int(self, value: int, length: int):
        self._output.write(value.to_bytes(length, byteorder="little"))


def encoder(
    value: S, encoder: Callable[[Serializer, S], bytes]
) -> bytes:
    ser = Serializer()
    encoder(ser, value)
    return ser.output()
