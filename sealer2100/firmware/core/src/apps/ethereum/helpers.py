...
if TYPE_CHECKING:
    from trezor.messages import EthereumFieldType, EthereumFieldTypeHypermate
...
def get_type_name_hypermate(field: EthereumFieldTypeHypermate) -> str:
    """Create a string from type definition (like uint256 or bytes16)."""
    from trezor.enums import EthereumDataTypeHypermate

    data_type = field.data_type
    size = field.size

    TYPE_TRANSLATION_DICT = {
        EthereumDataTypeHypermate.UINT: "uint",
        EthereumDataTypeHypermate.INT: "int",
        EthereumDataTypeHypermate.BYTES: "bytes",
        EthereumDataTypeHypermate.STRING: "string",
        EthereumDataTypeHypermate.BOOL: "bool",
        EthereumDataTypeHypermate.ADDRESS: "address",
    }

    if data_type == EthereumDataTypeHypermate.STRUCT:
        assert field.struct_name is not None  # validate_field_type
        return field.struct_name
    elif data_type == EthereumDataTypeHypermate.ARRAY:
        assert field.entry_type is not None  # validate_field_type
        type_name = get_type_name_hypermate(field.entry_type)
        if size is None:
            return f"{type_name}[]"
        else:
            return f"{type_name}[{size}]"
    elif data_type in (EthereumDataTypeHypermate.UINT, EthereumDataTypeHypermate.INT):
        assert size is not None  # validate_field_type
        return TYPE_TRANSLATION_DICT[data_type] + str(size * 8)
    elif data_type == EthereumDataTypeHypermate.BYTES:
        if size:
            return TYPE_TRANSLATION_DICT[data_type] + str(size)
        else:
            return TYPE_TRANSLATION_DICT[data_type]
