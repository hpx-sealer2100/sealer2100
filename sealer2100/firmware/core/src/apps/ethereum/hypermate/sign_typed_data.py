...
from trezor.enums import EthereumDataTypeHypermate as EthereumDataType
from trezor.ui import i18n
from trezor.messages import (
    EthereumFieldTypeHypermate as EthereumFieldType,
    EthereumSignTypedDataHypermate as EthereumSignTypedData,
    EthereumTypedDataSignatureHypermate as EthereumTypedDataSignature,
    EthereumTypedDataStructAckHypermate as EthereumTypedDataStructAck,
    EthereumTypedDataStructRequestHypermate as EthereumTypedDataStructRequest,
    EthereumTypedDataValueAckHypermate as EthereumTypedDataValueAck,
    EthereumTypedDataValueRequestHypermate as EthereumTypedDataValueRequest,
)
...
async def generate_typed_data_hash(
    ...
    if primary_type == "EIP712Domain":
        ...
        show_message = await should_show_struct(
            ...
            title=i18n.Title.confirm_message,
            button_text=i18n.Button.view_full_message,
        )
...
class TypedDataEnvelope:
    ...
    def encode_type(self, primary_type: str) -> bytes:
        ...
        for type_name in [primary_type] + sorted(deps):
            ...
            fields = ",".join(
                f"{get_type_name_hypermate(m.type)} {m.name}" for m in members
            )
            ...
            if field_type.data_type == EthereumDataType.STRUCT:
                ...
                if show_data:
                    show_struct = await should_show_struct(
                        ...
                        button_text=i18n.Button.view_full_struct,
                    )
                else:
                    show_struct = False
                ...
                if show_data:
                    show_array = await should_show_array(
                        ctx=self.ctx,
                        parent_objects=current_parent_objects,
                        data_type=get_type_name_hypermate(entry_type),
                        size=array_size,
                    )
                else:
                    show_array = False
                ...
                for i in range(array_size):
                    ...
                    if entry_type.data_type == EthereumDataType.STRUCT:
                        ...
                        if show_array:
                            await confirm_typed_value_hypermate(
                                ctx=self.ctx,
                                name=field_name,
                                value=value,
                                parent_objects=parent_objects,
