...
def _amount_ceil(amount: int, decimals) -> int:
    # 规则：以`decimlas`为精度为保留小数点后8位，如果小数点8位以后大于0,则第8位进
    if decimals <= 8:
        return amount
...
def format_amount_ceil(amount: int, decimals: int) -> str:
    if amount < 0:
        amount = -amount
        sign = "-"
    else:
    ...
    amount = _amount_ceil(amount, decimals)
    d = 10**decimals
    integer = amount // d
    decimal = amount % d
    ...
    decimal = f"{decimal:0{decimals}}"[:8]
    s = f"{sign}{grouped_integer}.{decimal}".rstrip("0").rstrip(".")
    return s
