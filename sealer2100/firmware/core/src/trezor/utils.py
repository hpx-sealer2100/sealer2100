...
DEFAULT_LABEL = "Sealer2100"
SHOW_APP_GUIDE_RESET = 1
SHOW_APP_GUIDE_RECOVER = 2
_SHOW_APP_GUIDE = 0
if __debug__:
    if EMULATOR:
        import uos
...
if TYPE_CHECKING:
    ...
    from typing import Any, Iterable, Iterator, Protocol, Sequence, TypeVar, Optional, Callable
...
def show_app_guide():
    ...
    return _SHOW_APP_GUIDE


def make_show_app_guide(typ: int):
    global _SHOW_APP_GUIDE
    _SHOW_APP_GUIDE = typ
...
def unimport_end(mods: set[str], collect: bool = True) -> None:
    ...
    assert len(sys.modules) <= 256, "Please bump preallocated size in mpconfigport.h"
...
def first(C: Iterable[T], F: Optional[Callable[[T], bool]] = None) -> Optional[T]:
    try:
        return next(filter(F, C))
    except StopIteration:
        return None

def count(init: int = 0, step: int = 1) -> Iterator[int]:
    """
    Make an iterator that returns evenly spaced values beginning with start.

    Like `itertools.count`
    """
    while True:
        yield init
        init += step
...
def enable_show_busy():
    global _show_busy
    _show_busy = True

def disable_show_busy():
    global _show_busy
    _show_busy = False
