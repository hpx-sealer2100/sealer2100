...
if TYPE_CHECKING:
    ...
    class Interface(Protocol):
        """
        Wire interface. It is responsible for reading and writing data over the wire.
        ...
        The `read` method should return a byte array containing data read from the wire. The
        `write` method should write the passed byte array to the wire.
        """
        async def read(self) -> bytes:
            """
            Reads data from the wire interface.

            This is an asynchronous operation that waits for the reader to be ready
            to receive data. The function returns the bytes read from the wire.

            Returns:
                bytes: The data read from the wire.
            """
            ...
        def write(self, data: bytes) -> int:
            """
            Writes data to the wire interface.

            This is a synchronous operation that blocks until the data is written
            to the wire.

            Args:
                data: The data to be written to the wire.

            Returns:
                int: The number of bytes written to the wire.
            """
            ...
...
class Wire():
    ...
    A wrapper for `WireInterface` that provides `read` and `write` methods.
    """
    def __init__(self, iface: WireInterface):
        from trezor import io
        self.iface = iface
        self.reader = loop.wait(self.iface.iface_num() | io.POLL_READ)
        self.writer = loop.wait(self.iface.iface_num() | io.POLL_WRITE)
...
def setup(iface: WireInterface, is_debug_session: bool = False) -> None:
    ...
    wire = Wire(iface)
    loop.schedule(handle_session(wire, codec_v1.SESSION_ID, is_debug_session))
    # async def session_task():
    #    while True:
    #        await handle_session(wire, codec_v1.SESSION_ID, is_debug_session)
    # loop.schedule(session_task())
...
class Context:
    def __init__(self, wire: Wire, sid: int, buffer: bytearray) -> None:
        self.wire = wire
        self.sid = sid
        self.buffer = buffer
    ...
    def read_from_wire(self) -> Awaitable[codec_v1.Message]:
        return codec_v1.read_message(self.wire, self.buffer)
...
async def _handle_single_message(
    ...
    handler = find_handler(ctx.wire.iface, msg.type)  # pylint: disable=assignment-from-none
    if handler is None:
    ...
    try:
        ...
        from trezor.utils import enable_show_busy
        enable_show_busy()
        res_msg = failure(exc)
...
async def handle_session(
    wire: Wire, session_id: int, is_debug_session: bool = False
) -> None:
    ...
    ctx = Context(wire, session_id, ctx_buffer)
