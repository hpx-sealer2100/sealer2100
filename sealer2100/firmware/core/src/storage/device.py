...
if TYPE_CHECKING:
    ...
    BTCAddressTypeValue = Literal[86, 44, 49, 84]
...
_BTC_ADDRESS_TYPE=const(0x8C)
SAFETY_CHECK_LEVEL_STRICT  : Literal[0] = const(0)
SAFETY_CHECK_LEVEL_PROMPT  : Literal[1] = const(1)
_DEFAULT_SAFETY_CHECK_LEVEL = SAFETY_CHECK_LEVEL_STRICT

DEVICE_PROTECT_TYPE_PIN : Literal[1] = const(1)
DEVICE_PROTECT_TYPE_IRIS: Literal[2] = const(2)
DEVICE_PROTECT_TYPE_PIN_AND_IRIS :Literal[3] = const(3)
_DEFAULT_PROTECT_TYPE = DEVICE_PROTECT_TYPE_PIN_AND_IRIS
if TYPE_CHECKING:
    StorageSafetyCheckLevel = Literal[0, 1]
    WeakupType = Literal[0, 1]
    DeviceProtectType = Literal[1,2,3]
# fmt: on
...
if __debug__:
    ...
    AUTOLOCK_DELAY_MINIMUM = AUTOSHUTDOWN_DELAY_MINIMUM = 30 * 1000  # 30 seconds

AUTOLOCK_DELAY_DEFAULT = AUTOSHUTDOWN_DELAY_DEFAULT = 2 * 60 * 1000  # 2 minutes
...
# default, weakup device when touch screen
DEVICE_WEAKUP_DEFAULT = 0
...
def set_airgap_status(enable: bool) -> None:
    set_ble_status(not enable)
...
def get_label() -> str:
    ...
    Returns:
        str: if label == "", return default label "HyperMate" instead
    """
...
def is_passphrase_enabled() -> bool:
    return common.get_bool(_NAMESPACE, _USE_PASSPHRASE, public=True)


def set_passphrase_enabled(enable: bool) -> None:
    common.set_bool(_NAMESPACE, _USE_PASSPHRASE, enable, public=True)
    if not enable:
...
def get_autolock_delay_ms() -> int:
    b = common.get(_NAMESPACE, _AUTOLOCK_DELAY_MS, public=True)
    if b is None:
...
def set_autolock_delay_ms(delay_ms: int) -> None:
    common.set(_NAMESPACE, _AUTOLOCK_DELAY_MS, struct.pack('>l', delay_ms), public=True)
...
def set_autoshutdown_delay_ms(delay_ms: int) -> None:
    common.set(
        _NAMESPACE, _AUTOSHUTDOWN_DELAY_MS, struct.pack('>l', delay_ms), public=True
    )
...
def get_device_weakup() -> WeakupType:
    b = common.get(_NAMESPACE, _DEVICE_WEAKUP, public=True)
    if b is None:
        return DEVICE_WEAKUP_DEFAULT
    else:
        return int.from_bytes(b, 'big')

def set_device_weakup(weakup: WeakupType):
    common.set(_NAMESPACE, _DEVICE_WEAKUP, weakup.to_bytes(1, 'big'), public=True)

def get_device_protect_type() -> DeviceProtectType:
    b = common.get(_NAMESPACE, _PROTECT_TYPE, public=True)
    if b is None:
        return _DEFAULT_PROTECT_TYPE

    return int.from_bytes(b, 'big')

def set_device_protect_type(protect: DeviceProtectType):
    common.set(_NAMESPACE, _PROTECT_TYPE, protect.to_bytes(1, 'big'), public=True)

def get_defi_lock() -> bool:
    return common.get_bool(_NAMESPACE, _DEFI_LOCK)
...
class BTCAddressType:
    ...
    def __eq__(self, value: 'BTCAddressType'):
    ...
    def path(self):
        return f"m/{self.type}'/0'/0'/0/0"

    def script_type(self) -> 'InputScriptType':
...
def get_btc_address_type() -> BTCAddressType:
    b = common.get(_NAMESPACE, _BTC_ADDRESS_TYPE, public=True)
    if b is None:
        return BTCAddressType(86)

    return BTCAddressType(int.from_bytes(b, 'big'))

def set_btc_address_type(type: BTCAddressType):
    common.set(_NAMESPACE, _BTC_ADDRESS_TYPE, type.type.to_bytes(1, 'big'), public=True)