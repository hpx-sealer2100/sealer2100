...
if [[ $RUN_TEST_EMU > 0 ]]; then
  ...
  mv "${TREZOR_PROFILE_DIR}/HyperMate.flash" "${TREZOR_PROFILE_DIR}/HyperMate.flash.bkp" 2>/dev/null
  mv "${TREZOR_PROFILE_DIR}/onkey.sdcard" "${TREZOR_PROFILE_DIR}/HyperMate.sdcard.bkp" 2>/dev/null
