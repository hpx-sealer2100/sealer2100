...
3. start the bridge:
   - install [Trezor Bridge](https://Trezor.so/download/)
   - alternatively, launch the [desktop suite](https://Trezor.so/download/) from the command line with the argument `--bridge-dev`

Now you can use the emulator the same way as you use the device, for example you can use [Trezor Suite](https://Trezor.so/download/), use our Python CLI tool (`trezorctl`), etc. Simply click to emulate screen touches.
...
red square in the upper right corner of HyperMate screen.
...
Internal HyperMate's storage is emulated and stored in the `/var/tmp/HyperMate.flash` file by
default. Deleting this file is similar to calling _wipe device_. You can also find
`/var/tmp/HyperMate.sdcard` for SD card.
...
This will create a profile directory in your home `~/.HyperMateemu/foobar` containing
