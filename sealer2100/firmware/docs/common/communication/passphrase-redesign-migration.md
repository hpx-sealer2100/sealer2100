...
On the T1, passphrase must be entered on the host PC and sent to HyperMate. On the TT, the
user can choose whether to enter the passphrase on host or on HyperMate's touch screen.
...
contain the `Capability_PassphraseEntry` value, regardless of reported HyperMate version or
