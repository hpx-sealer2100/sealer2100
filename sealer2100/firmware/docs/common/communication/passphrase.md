...
As of 1.9.0 / 2.3.0 we have changed how passphrase is communicated between the host and the device. For migration information for existing Hosts communicating with HyperMate please see this [document](passphrase-redesign-migration.md).
...
As soon as HyperMate needs the passphrase to do BIP-39/SLIP-39 derivations it prompts the user for passphrase.
...
In the default HyperMate setting, the passphrase is obtained from the Host. HyperMate sends a PassphraseRequest message and awaits PassphraseAck as a response. This message contains field `passphrase` to transmit it or it has `on_device` boolean flag to indicate that the user wishes to enter the passphrase on HyperMate instead. Setting both `passphrase` and `on_device` to true is forbidden.

Note that this has changed as of 2.3.0. In previous firmware versions the `on_device` flag was in the PassphraseRequest message, since this decision has been made on HyperMate. We also had two additional messages PassphraseStateRequest and PassphraseStateAck which were removed.
...
As long as the session_id in `Initialize` is the same as the one HyperMate stores internally, HyperMate guarantees the same passphrase is being used.
