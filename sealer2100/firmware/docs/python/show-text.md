# Displaying text screens on HyperMate T

## Requirements

For this feature, you will need a debug-enabled firmware for HyperMate T. Usually that
will be the emulator.

You will also need `trezorctl` 0.12 or later. Best bet is to run the version from github
master.

## HyperMate T text capabilities

The HyperMate T screen has a _header_, with an icon and text. Below it is _body_, which
