...
In HyperMate T this works a bit differently, we have introduced so-called vendors headers.
Each firmware has its vendor header and this vendor header is signed by HyperMate. The
actual firmware is signed by the vendor header's key. That means that all firmwares are
signed by _someone_ to be able to run on HyperMate T.
...
As the names suggest, the first one is the official HyperMate vendor header and all
...
(HyperMate) and you install the unofficial (UNSIGNED) firmware -> the device gets
